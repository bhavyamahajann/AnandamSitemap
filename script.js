...
// SECTION 2 : Below 174-179 AND parallel to 166-154, 143-142 section
{
  ...
  const plot170X = block1X + (3 * (PW + GAP)); // Plot 170 is 4th plot (0-indexed: 3)
  const midX = plot170X;
  ...
  const b9 = grid2col(b9X, b9Y, block9L, block9R, CORR, PW, 56); 
  world.push(b9.svg);
...
{
  const topY = y - 150; // Shifted up to align block11 with block9 (plots 100-102)
  ...
  const plot76BottomY = rightY + PH + RCORR; // Y position where plot 76 ends (bottom row bottom)
  const plot77Y = plot76BottomY + 20; // Start below plot 76 with gap
  ...
  const col19Y = plot77Y + plot77Height + 30; // Below plot 77 with proper gap
